...
export class NestedHistogram {
   ...
   init() {
      ...
      console.log(this.totalInstances)
   ...
   renderHistogram(startIndex, endIndex, layer) {
      ...
      const render = (startIndex, endIndex, currentLayer, obj, limits) => {
         ...
         for (let i = startIndex; i < endIndex; i += stepFor) {
            ...
            this.color = new THREE.Color(counter.getIndex() / 100, 0, 1 - counter.getIndex() / 10);
            ...
            if (currentLayer === layer) {
               ...
               if (obj.children.content){
                  ...
                  child = obj.children[this.selectedChildren[0]][index];
               }
               // console.log(child)
      ...
      const rec = (layer, obj, index) => {
         ...
         if (layer + 1 < position.length) {
            ...
            if (obj.children.content){
               ...
               child = obj.children[this.selectedChildren[0]][obj.getBin(position[layer].x + 1, position[layer].y + 1, position[layer].z + 1)];
            }
            if (index > layer) {
   ...
   getChildObject(obj, indexLayer) {
      ...
      if (obj.children.content) {
         ...
         return obj.children[this.selectedChildren[0]][binIndex];
      }
   }
            ...
            validY.forEach(y => {
               ...
               validX.forEach(x => {
                  ...
                  if (node.children?.content) {
                     ...
                     children = node.children?.[this.selectedChildren];
                  }
